...
class Board:
        ...
        for y in range(self.x):
            for x in range(self.x):
                if self.ValidMove(board, x, y, player):
                    ...
                    points = self.AlphaBeta(board, player, self.depth, self.minEvalBoard, self.maxEvalBoard, True)
                    if points > maxPoints:
                        maxPoints = points
                        mx = x
                        my = y
    ...
    def AlphaBeta(self, board, player, depth, alpha, beta, maximizingPlayer):
        ...
        v = self.minEvalBoard if maximizingPlayer else self.maxEvalBoard
        for y in range(self.x):
            for x in range(self.x):
                if self.ValidMove(board, x, y, player):
                    (boardTemp, totctr) = self.make_move(copy.deepcopy(board), x, y, player)
